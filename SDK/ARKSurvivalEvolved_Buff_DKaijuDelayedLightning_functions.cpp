// ARKSurvivalEvolved (301.1) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "ARKSurvivalEvolved_Buff_DKaijuDelayedLightning_parameters.hpp"

namespace sdk
{
//---------------------------------------------------------------------------
//Functions
//---------------------------------------------------------------------------

// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPPreSetupForInstigator
// ()
// Parameters:
// class AActor**                 ForInstigator                  (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::BPPreSetupForInstigator(class AActor** ForInstigator)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPPreSetupForInstigator");

	ABuff_DKaijuDelayedLightning_C_BPPreSetupForInstigator_Params params;
	params.ForInstigator = ForInstigator;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPActivated
// ()
// Parameters:
// class AActor**                 ForInstigator                  (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::BPActivated(class AActor** ForInstigator)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPActivated");

	ABuff_DKaijuDelayedLightning_C_BPActivated_Params params;
	params.ForInstigator = ForInstigator;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPDeactivated
// ()
// Parameters:
// class AActor**                 ForInstigator                  (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::BPDeactivated(class AActor** ForInstigator)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPDeactivated");

	ABuff_DKaijuDelayedLightning_C_BPDeactivated_Params params;
	params.ForInstigator = ForInstigator;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BuffTickServer
// ()
// Parameters:
// float*                         DeltaTime                      (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::BuffTickServer(float* DeltaTime)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BuffTickServer");

	ABuff_DKaijuDelayedLightning_C_BuffTickServer_Params params;
	params.DeltaTime = DeltaTime;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPSetupForInstigator
// ()
// Parameters:
// class AActor**                 ForInstigator                  (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::BPSetupForInstigator(class AActor** ForInstigator)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.BPSetupForInstigator");

	ABuff_DKaijuDelayedLightning_C_BPSetupForInstigator_Params params;
	params.ForInstigator = ForInstigator;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.MaybeRotate
// ()

void ABuff_DKaijuDelayedLightning_C::MaybeRotate()
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.MaybeRotate");

	ABuff_DKaijuDelayedLightning_C_MaybeRotate_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.UserConstructionScript
// ()

void ABuff_DKaijuDelayedLightning_C::UserConstructionScript()
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.UserConstructionScript");

	ABuff_DKaijuDelayedLightning_C_UserConstructionScript_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.SetStructure
// ()
// Parameters:
// class APrimalStructure*        NewParam                       (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::SetStructure(class APrimalStructure* NewParam)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.SetStructure");

	ABuff_DKaijuDelayedLightning_C_SetStructure_Params params;
	params.NewParam = NewParam;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.Client_DeactivateParticle
// ()

void ABuff_DKaijuDelayedLightning_C::Client_DeactivateParticle()
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.Client_DeactivateParticle");

	ABuff_DKaijuDelayedLightning_C_Client_DeactivateParticle_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ReceiveBeginPlay
// ()

void ABuff_DKaijuDelayedLightning_C::ReceiveBeginPlay()
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ReceiveBeginPlay");

	ABuff_DKaijuDelayedLightning_C_ReceiveBeginPlay_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ReceiveTick
// ()
// Parameters:
// float*                         DeltaSeconds                   (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::ReceiveTick(float* DeltaSeconds)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ReceiveTick");

	ABuff_DKaijuDelayedLightning_C_ReceiveTick_Params params;
	params.DeltaSeconds = DeltaSeconds;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ExecuteUbergraph_Buff_DKaijuDelayedLightning
// ()
// Parameters:
// int                            EntryPoint                     (Parm, ZeroConstructor, IsPlainOldData)

void ABuff_DKaijuDelayedLightning_C::ExecuteUbergraph_Buff_DKaijuDelayedLightning(int EntryPoint)
{
	static auto fn = UObject::FindObject<UFunction>("Function Buff_DKaijuDelayedLightning.Buff_DKaijuDelayedLightning_C.ExecuteUbergraph_Buff_DKaijuDelayedLightning");

	ABuff_DKaijuDelayedLightning_C_ExecuteUbergraph_Buff_DKaijuDelayedLightning_Params params;
	params.EntryPoint = EntryPoint;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
