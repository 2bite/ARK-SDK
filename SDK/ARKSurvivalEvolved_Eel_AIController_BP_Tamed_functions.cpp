// ARKSurvivalEvolved (318.14) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "ARKSurvivalEvolved_Eel_AIController_BP_Tamed_parameters.hpp"

namespace sdk
{
//---------------------------------------------------------------------------
//Functions
//---------------------------------------------------------------------------

// Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.BPGetTargetingDesire
// ()
// Parameters:
// class AActor**                 forTarget                      (Parm, ZeroConstructor, IsPlainOldData)
// float*                         ForTargetingDesireValue        (Parm, ZeroConstructor, IsPlainOldData)
// float                          ReturnValue                    (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData)

float AEel_AIController_BP_Tamed_C::BPGetTargetingDesire(class AActor** forTarget, float* ForTargetingDesireValue)
{
	static auto fn = UObject::FindObject<UFunction>("Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.BPGetTargetingDesire");

	AEel_AIController_BP_Tamed_C_BPGetTargetingDesire_Params params;
	params.forTarget = forTarget;
	params.ForTargetingDesireValue = ForTargetingDesireValue;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;

	return params.ReturnValue;
}


// Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.UserConstructionScript
// ()

void AEel_AIController_BP_Tamed_C::UserConstructionScript()
{
	static auto fn = UObject::FindObject<UFunction>("Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.UserConstructionScript");

	AEel_AIController_BP_Tamed_C_UserConstructionScript_Params params;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.ExecuteUbergraph_Eel_AIController_BP_Tamed
// ()
// Parameters:
// int                            EntryPoint                     (Parm, ZeroConstructor, IsPlainOldData)

void AEel_AIController_BP_Tamed_C::ExecuteUbergraph_Eel_AIController_BP_Tamed(int EntryPoint)
{
	static auto fn = UObject::FindObject<UFunction>("Function Eel_AIController_BP_Tamed.Eel_AIController_BP_Tamed_C.ExecuteUbergraph_Eel_AIController_BP_Tamed");

	AEel_AIController_BP_Tamed_C_ExecuteUbergraph_Eel_AIController_BP_Tamed_Params params;
	params.EntryPoint = EntryPoint;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
