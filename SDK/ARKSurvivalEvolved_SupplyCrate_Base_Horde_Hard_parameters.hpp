#pragma once

// ARKSurvivalEvolved (318.14) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "ARKSurvivalEvolved_SupplyCrate_Base_Horde_Hard_classes.hpp"

namespace sdk
{
//---------------------------------------------------------------------------
//Parameters
//---------------------------------------------------------------------------

// Function SupplyCrate_Base_Horde_Hard.SupplyCrate_Base_Horde_Hard_C.OnRep_bHasLanded
struct ASupplyCrate_Base_Horde_Hard_C_OnRep_bHasLanded_Params
{
};

// Function SupplyCrate_Base_Horde_Hard.SupplyCrate_Base_Horde_Hard_C.UserConstructionScript
struct ASupplyCrate_Base_Horde_Hard_C_UserConstructionScript_Params
{
};

// Function SupplyCrate_Base_Horde_Hard.SupplyCrate_Base_Horde_Hard_C.ExecuteUbergraph_SupplyCrate_Base_Horde_Hard
struct ASupplyCrate_Base_Horde_Hard_C_ExecuteUbergraph_SupplyCrate_Base_Horde_Hard_Params
{
	int                                                EntryPoint;                                               // (Parm, ZeroConstructor, IsPlainOldData)
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
