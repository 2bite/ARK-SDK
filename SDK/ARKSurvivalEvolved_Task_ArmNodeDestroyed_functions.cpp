// ARKSurvivalEvolved (301.1) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x8)
#endif

#include "ARKSurvivalEvolved_Task_ArmNodeDestroyed_parameters.hpp"

namespace sdk
{
//---------------------------------------------------------------------------
//Functions
//---------------------------------------------------------------------------

// Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ReceiveExecute
// ()
// Parameters:
// class AActor**                 OwnerActor                     (Parm, ZeroConstructor, IsPlainOldData)

void UTask_ArmNodeDestroyed_C::ReceiveExecute(class AActor** OwnerActor)
{
	static auto fn = UObject::FindObject<UFunction>("Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ReceiveExecute");

	UTask_ArmNodeDestroyed_C_ReceiveExecute_Params params;
	params.OwnerActor = OwnerActor;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ReceiveAbort
// ()
// Parameters:
// class AActor**                 OwnerActor                     (Parm, ZeroConstructor, IsPlainOldData)

void UTask_ArmNodeDestroyed_C::ReceiveAbort(class AActor** OwnerActor)
{
	static auto fn = UObject::FindObject<UFunction>("Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ReceiveAbort");

	UTask_ArmNodeDestroyed_C_ReceiveAbort_Params params;
	params.OwnerActor = OwnerActor;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


// Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ExecuteUbergraph_Task_ArmNodeDestroyed
// ()
// Parameters:
// int                            EntryPoint                     (Parm, ZeroConstructor, IsPlainOldData)

void UTask_ArmNodeDestroyed_C::ExecuteUbergraph_Task_ArmNodeDestroyed(int EntryPoint)
{
	static auto fn = UObject::FindObject<UFunction>("Function Task_ArmNodeDestroyed.Task_ArmNodeDestroyed_C.ExecuteUbergraph_Task_ArmNodeDestroyed");

	UTask_ArmNodeDestroyed_C_ExecuteUbergraph_Task_ArmNodeDestroyed_Params params;
	params.EntryPoint = EntryPoint;

	auto flags = fn->FunctionFlags;

	UObject::ProcessEvent(fn, &params);

	fn->FunctionFlags = flags;
}


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
